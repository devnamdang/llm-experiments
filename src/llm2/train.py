...
#import src.llm1.model as mdl
#import src.data as dt
import src.llm2.model as mdl
import src.data as dt
#%% Constants
MODEL_NAME = "llm2"
CONFIG_PATH = Path("config/config_llm2.yaml")
...
def save_output(model, optimizer, fig, losses_df):
    # Create output dir
    output_run_dir = OUTPUT_DIR / (MODEL_NAME + "_" + datetime.datetime.now().strftime("%Y%m%d%H%M%S"))
...
def run():
    ...
    nr_params = count_nr_parameters(model)
    print(f"Model number of parameters: {nr_params}")
